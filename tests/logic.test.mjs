...
test("canvas files open as editable React Flow graph tabs", () => {
  ...
  assert.match(app, /import \{ CanvasView, canvasTitle, isCanvasPath \} from "\.\/CanvasView"/);
  ...
  assert.match(app, /<CanvasView\s+content=\{paneMarkdown\}/);
  ...
  assert.match(canvasNodes, /"--canvas-node-bg": `color-mix\(in srgb, \$\{accent\} 24%, var\(--surface\)\)`/);
  assert.match(canvasNodes, /"--canvas-node-border": `color-mix\(in srgb, \$\{accent\} 78%, var\(--border\)\)`/);
  ...
  assert.match(canvasLogic, /const canvasEdgeStrokeWidth = 2\.5/);
  assert.match(canvasView, /MarkerType\.ArrowClosed/);
  assert.match(canvasView, /markerEnd: canvasEdgeMarker/);
  assert.match(canvasView, /strokeWidth: canvasEdgeStrokeWidth/);
  ...
  assert.match(css, /background: var\(--canvas-node-bg/);
  assert.match(css, /border: 1\.5px solid var\(--canvas-node-border/);
