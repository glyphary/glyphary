...
test("quick command palette exposes initial editor commands", () => {
  ...
  assert.match(paletteDefinitions, /insertCommandPaletteCommands/);
  assert.match(paletteDefinitions, /canvasInsertCommandPaletteCommands/);
  ...
  assert.match(paletteDefinitions, /activeDocumentIsCanvas\s*\?\s*canvasInsertCommandPaletteCommands\s*:\s*activeDocumentIsMarkdown\s*\?\s*insertCommandPaletteCommands\s*:\s*\[\]/);
  ...
  assert.match(paletteDefinitions, /id: "create-tidbit"/);
  assert.match(paletteDefinitions, /title: "Create Tidbit"/);
  ...
  assert.match(paletteDefinitions, /title: "Insert rich link"/);
  assert.match(paletteDefinitions, /id: "insert-columns"/);
  ...
  assert.match(paletteDefinitions, /id: "insert-callout"/);
  ...
  assert.match(paletteDefinitions, /id: "insert-html-block"/);
  assert.match(app, /title: "Insert HTML block"/);
  assert.match(paletteDefinitions, /id: "insert-mermaid-diagram"/);
  assert.match(app, /title: "Insert Mermaid diagram"/);
  assert.match(app, /function insertMermaidDiagram\(\)/);
  assert.match(app, /flowchart TD\\n  A\[Start\] --> B\[Done\]/);
  assert.match(paletteDefinitions, /id: "insert-table-of-contents"/);
  assert.match(paletteDefinitions, /title: "Insert table of contents"/);
  assert.match(paletteDefinitions, /id: "insert-menu"/);
  assert.match(paletteDefinitions, /title: "Insert \.\.\."/);
  ...
  assert.match(paletteDefinitions, /id: "format-strikethrough"/);
  assert.match(paletteDefinitions, /title: "Strikethrough"/);
  assert.match(paletteDefinitions, /formatSelectionWithMark\(editor, "strike", "strikethrough", setStatus\)/);
  assert.match(paletteDefinitions, /id: "format-highlight"/);
  assert.match(paletteDefinitions, /title: "Highlight"/);
  assert.match(paletteDefinitions, /formatSelectionWithMark\(editor, "highlight", "a highlight", setStatus\)/);
  assert.match(paletteDefinitions, /id: "format-superscript"/);
  assert.match(paletteDefinitions, /title: "Superscript"/);
  assert.match(paletteDefinitions, /formatSelectionWithMark\(editor, "superscript", "superscript", setStatus\)/);
  assert.match(paletteDefinitions, /id: "format-subscript"/);
  assert.match(paletteDefinitions, /title: "Subscript"/);
  assert.match(paletteDefinitions, /formatSelectionWithMark\(editor, "subscript", "subscript", setStatus\)/);
  assert.match(paletteDefinitions, /id: "format-keyboard"/);
  assert.match(paletteDefinitions, /title: "Keyboard"/);
  assert.match(app, /formatSelectionWithMark\(editor, "keyboardKey", "a keyboard key", setStatus\)/);
  assert.match(paletteDefinitions, /Wrap selected text in <kbd> tags/);
  assert.match(css, /\.editor-surface mark/);
  assert.match(css, /\.editor-surface sup,/);
  assert.match(css, /\.editor-surface sub/);
  assert.match(paletteDefinitions, /id: "format-menu"/);
  assert.match(paletteDefinitions, /title: "Format \.\.\."/);
  assert.match(paletteDefinitions, /id: "canvas-add-card"/);
  assert.match(paletteDefinitions, /title: "Add Card"/);
  assert.match(paletteDefinitions, /id: "canvas-add-note-from-vault"/);
  assert.match(paletteDefinitions, /title: "Add Note From Vault"/);
  assert.match(paletteDefinitions, /id: "canvas-add-media-from-vault"/);
  assert.match(paletteDefinitions, /title: "Add Media From Vault"/);
  assert.match(paletteDefinitions, /id: "canvas-add-web-page"/);
  assert.match(paletteDefinitions, /title: "Add Web Page"/);
  assert.match(paletteDefinitions, /id: "canvas-create-group"/);
  assert.match(paletteDefinitions, /title: "Create Group"/);
  assert.match(paletteDefinitions, /setCommandPaletteScope\("insert"\)/);
  ...
  assert.match(paletteDefinitions, /setCommandPaletteScope\("format"\)/);
...
test("AI commands use vault settings and review output before editing", () => {
  ...
  assert.match(paletteDefinitions, /id: "ai-page-builder"/);
  assert.match(aiImplementation, /AI: Page Builder/);
  assert.match(paletteDefinitions, /id: "ai-improve-writing-selection"/);
  assert.match(paletteDefinitions, /AI: Improve writing/);
  assert.match(paletteDefinitions, /id: "ai-fix-spelling-grammar-selection"/);
  assert.match(paletteDefinitions, /id: "ai-shorten-selection"/);
  assert.match(paletteDefinitions, /id: "ai-expand-selection"/);
  assert.match(paletteDefinitions, /id: "ai-tone-formal-selection"/);
  ...
  assert.match(paletteDefinitions, /id: "ai-diagram-selection"/);
  assert.match(
    paletteDefinitions,
    /id: "ai-diagram-selection",[\s\S]{0,900}?runAiSelectionOrDocumentCommand\([\s\S]{0,900}?"insert-below-selection",/,
  );
  assert.match(paletteDefinitions, /Return only one fenced ```mermaid code block/);
  assert.match(paletteDefinitions, /id: "ai-generate-title"/);
  assert.match(paletteDefinitions, /id: "ai-continue-writing"/);
  assert.match(paletteDefinitions, /id: "ai-explain-selection"/);
  assert.match(paletteDefinitions, /const aiCommandPaletteCommands: CommandPaletteCommand\[\] = savedAiSettings\(\)\.enabled/);
  assert.match(paletteDefinitions, /id: "ai-menu"/);
  assert.match(paletteDefinitions, /title: "AI \.\.\."/);
  assert.match(paletteDefinitions, /setCommandPaletteScope\("ai"\)/);
...
test("table row and column actions are contextual command palette entries", () => {
  ...
  assert.match(paletteDefinitions, /title: "Add row after"/);
  assert.match(app, /id: "table-delete-row"/);
  assert.match(paletteDefinitions, /title: "Delete row"/);
  assert.match(app, /id: "table-add-column-after"/);
  assert.match(paletteDefinitions, /title: "Add column after"/);
  assert.match(app, /id: "table-delete-column"/);
  assert.match(paletteDefinitions, /title: "Delete column"/);
  assert.match(app, /id: "table-delete-table"/);
  assert.match(paletteDefinitions, /title: "Delete table"/);
  assert.match(paletteDefinitions, /id: "table-menu"/);
  assert.match(paletteDefinitions, /title: "Table \.\.\."/);
  assert.match(paletteDefinitions, /setCommandPaletteScope\("table"\)/);
